...
<img src="../assets/taxonomy.svg" alt="Taxonomy: inputs (prompt, document, paper, existing deck) flow through methods (pipeline, multi-agent, code generation, diffusion) to outputs (slide deck, poster, layout, video)" width="100%">

## Papers at a glance: model × method

Within the papers, two more axes are largely **orthogonal**: which *backbone model* a system builds on (off-the-shelf text LLM, multimodal LLM/VLM, a fine-tuned specialized model, or a diffusion model) and which *generation method* it uses (staged pipeline, multi-agent iteration, code generation, RL post-training, or direct diffusion generation). Placing representative papers on this grid shows where the field clusters — and where it is still empty:

<img src="../assets/paper-matrix.svg" alt="Matrix of representative papers by backbone model (rows) and generation method (columns), chip color encodes output type" width="100%">

A few patterns worth noticing:

- **Multi-agent systems on multimodal LLMs** are the current center of gravity — reflection on *rendered* slides (DeepPresenter, EvoPresent) requires a model that can see its own output.
- **Code generation pairs naturally with fine-tuning** (AutoPresent, SlideCoder, DeepSlides): emitting `python-pptx`/HTML makes output verifiable, which in turn makes training signals cheap.
- **Diffusion models stay in their own row and column** — they generate layouts and artistic posters directly, and are combined with LLMs (POSTA, PosterVerse) rather than replaced by them.
- **RL post-training is a layer, not a paradigm** — it appears wherever a reward is computable (aesthetic scores in EvoPresent, geometric layout rewards in AeSlides/LaySPA).

> Both diagrams are generated by [`scripts/make_diagrams.py`](../scripts/make_diagrams.py) — when adding a paper, extend the `PLACEMENTS` table there and re-run it.
